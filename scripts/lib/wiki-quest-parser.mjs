 ...
 * the quest log yet.
 */
export function isQuestReleased(releaseDateText, now = new Date()) {
 ...
 * released status, start, description, requirements, enemies, items
 * required, wiki link), by scraping the `{{Infobox Quest}}`,
 * `{{Quest details}}`, and `{{Quest rewards}}` templates.
 */
