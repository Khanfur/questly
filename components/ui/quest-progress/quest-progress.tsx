...
interface QuestProgressProps extends Omit<React.ComponentProps<typeof Progress>, 'children' | 'value'> {
    questName: string
    status: QuestProgressStatus;
}

function QuestProgress({ questName, status, className, ...props }: QuestProgressProps) {
    return (
        <div className="flex flex-col gap-1">
            <span className="label">{questName} — {status}</span>
